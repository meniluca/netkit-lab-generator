...
foreach $line (@lines)
{
    ...
    my $string_startup = `kdialog --inputbox "$name" "$ifconfig_string"`;
...
foreach my $name ( @unique_ws ) {
    ...
    `echo "<h1>Stai visualizzando il webserver situato su: $name</h1>" >> $name/var/www/index.html`;
    `echo "</body>" >> $name/var/www/index.html`;
    `echo "</html>" >> $name/var/www/index.html`;
}
...
foreach my $name ( @unique_bgp ) {
    ...
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!hostname bgpd" >> $name/etc/zebra/bgpd.conf`;
    `echo "!password zebra" >> $name/etc/zebra/bgpd.conf`;
    `echo "!enable password zebra" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!log file /var/log/zebra/bgpd.log" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "debug bgp" >> $name/etc/zebra/bgpd.conf`;
    `echo "debug bgp events" >> $name/etc/zebra/bgpd.conf`;
    `echo "debug bgp filters" >> $name/etc/zebra/bgpd.conf`;
    `echo "debug bgp fsm" >> $name/etc/zebra/bgpd.conf`;
    `echo "debug bgp keepalives" >> $name/etc/zebra/bgpd.conf`;
    `echo "debug bgp updates" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!router bgp 1" >> $name/etc/zebra/bgpd.conf`;
    `echo "!network 195.11.14.0/24" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 193.10.11.2 remote-as 2" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 193.10.11.2 default-originate" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!router bgp 300" >> $name/etc/zebra/bgpd.conf`;
    `echo "!network 200.1.0.0/16" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.14 remote-as 40" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.14 description Router as40r1" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.14 prefix-list mineOutOnly out" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.14 prefix-list defaultIn in" >> $name/etc/zebra/bgpd.conf`;
    `echo "!ip prefix-list mineOutOnly permit 200.1.0.0/16" >> $name/etc/zebra/bgpd.conf`;
    `echo "!ip prefix-list mineOutOnly permit 200.1.128.0/17" >> $name/etc/zebra/bgpd.conf`;
    `echo "!ip prefix-list defaultIn permit 0.0.0.0/0" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "! as stub primary e backup" >> $name/etc/zebra/bgpd.conf`;
    `echo "!router bgp 100" >> $name/etc/zebra/bgpd.conf`;
    `echo "!network 100.1.0.0/16" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.2 remote-as 20" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.2 description Router as20r2 (primary)" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.2 prefix-list mineOutOnly out" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.2 prefix-list defaultIn in" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.6 remote-as 20" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.6 description Router as20r1 (backup)" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.6 prefix-list mineOutOnly out" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.6 route-map metricOut out" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.6 prefix-list defaultIn in" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 11.0.0.6 route-map localPrefIn in" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!ip prefix-list mineOutOnly permit 100.1.0.0/16" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!ip prefix-list defaultIn permit 0.0.0.0/0" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!route-map metricOut permit 10" >> $name/etc/zebra/bgpd.conf`;
    `echo "!match ip address myAggregate" >> $name/etc/zebra/bgpd.conf`;
    `echo "!set metric 10" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!route-map localPrefIn permit 10" >> $name/etc/zebra/bgpd.conf`;
    `echo "!set local-preference 90" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!access-list myAggregate permit 100.1.0.0/16" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "! esempio utilizzo route-map" >> $name/etc/zebra/bgpd.conf`;
    `echo "!router bgp 100" >> $name/etc/zebra/bgpd.conf`;
    `echo "!network 100.1.1.0/24" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 222.2.2.2 remote-as 200" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 222.2.2.2 route-map myRouteMap in" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!route-map myRouteMap permit 10" >> $name/etc/zebra/bgpd.conf`;
    `echo "!match ip address myAccessList" >> $name/etc/zebra/bgpd.conf`;
    `echo "!set metric 5" >> $name/etc/zebra/bgpd.conf`;
    `echo "!set local-preference 25" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!route-map myRouteMap permit 20" >> $name/etc/zebra/bgpd.conf`;
    `echo "!set metric 2" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!access-list myAccessList permit 193.204.0.0/16" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "! as-path filtering example" >> $name/etc/zebra/bgpd.conf`;
    `echo "! accept from as 200 only the routes" >> $name/etc/zebra/bgpd.conf`;
    `echo "!received via as 300" >> $name/etc/zebra/bgpd.conf`;
    `echo "!router bgp 100" >> $name/etc/zebra/bgpd.conf`;
    `echo "!network 100.1.1.0/24" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 222.2.2.2 remote-as 200" >> $name/etc/zebra/bgpd.conf`;
    `echo "!neighbor 222.2.2.2 filter-list myACL in" >> $name/etc/zebra/bgpd.conf`;
    `echo "!" >> $name/etc/zebra/bgpd.conf`;
    `echo "!ip as-path access-list myACL permit ^200_300" >> $name/etc/zebra/bgpd.conf`;
